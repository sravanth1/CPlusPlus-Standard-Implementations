...
void update(int index, int pos, long val, int a, int b){
	if(a==b && a==pos){
		tree[index]=makeNode(val, val, val, val);
		return;
	}
	int mid=(a+b)/2;
	if(pos<=mid){
	...
	tree[index]=makeNode(left.sum+right.sum, max(left.best, max(right.best, left.right+right.left)), max(left.left, left.sum+right.left), max(right.right, right.sum+left.right));
}
